...
import fire
import traceback
from atm5_model_lgb_chip import atm5_ModelLGB
from atm5_runner_chip import atm5_Runner
...
def main(mode='prd', create_features=True, model_type='lgb') -> str:
    ...
    cv = {
        ...
        'shuffle': True,
        'cv_target_gr': 'hoge',
        'cv_target_sf': 'chip_exc_wl_label'
    }

    # 学習・推論 LightGBM ###################################
    # run nameの設定
    run_name = 'lgb'
    run_name = run_name + '_' + cv.get('method') + suffix + '_chip_exc_wl_label'
    ...
    try:
        ...
        # Submission.create_submission(run_name, dir_name, 'target')  # submit作成
